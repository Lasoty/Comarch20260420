using CarSharing.Modules.Notifications.Domain;
using CarSharing.Modules.Notifications.Infrastructure;
using CarSharing.Modules.Reservations.Contracts;
using MediatR;

namespace CarSharing.Modules.Notifications.Application;

public sealed class ReservationConfirmedNotificationHandler(NotificationsDbContext dbContext)
    : INotificationHandler<ReservationConfirmed>
{
    public async Task Handle(ReservationConfirmed notification, CancellationToken cancellationToken)
    {
        var message =
            $"Reservation {notification.ReservationId} confirmed for user {notification.UserId}. Price: {notification.Price}.";

        dbContext.NotificationLogs.Add(
            new NotificationLog(Guid.NewGuid(), message, DateTime.UtcNow));

        await dbContext.SaveChangesAsync(cancellationToken);
    }
}
